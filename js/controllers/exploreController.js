var exploreController = controllers.controller("ExploreCtrl", function($scope,
				...
				.then(
						function(resultData) {
							...
							MyFamilyService.getViewDocuments(
									...
									$scope.keywords).then(function(resultData) {
	...
	function init() {
		...
		var urlKeywords = parseInt($location.search().keywords, 10);
		...
		var urlCurrentPage = parseInt($location.search().page, 10);
		...
		var urlPageSize = parseInt($location.search().pageSize, 10);
		...
		$scope.$watch('keywords', function(name) {
